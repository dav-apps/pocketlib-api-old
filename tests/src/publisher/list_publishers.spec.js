...
describe("ListPublishers endpoint", () => {
	...
	it("should return publishers of admin", async () => {
		...
		assert.equal(response.data.items.length, constants.davUser.publishers.length)

		for (let publisher of constants.davUser.publishers) {
			let responsePublisher = response.data.items.find(p => p.uuid == publisher.uuid)

			assert.isNotNull(responsePublisher)
			assert.equal(Object.keys(responsePublisher).length, 8)
