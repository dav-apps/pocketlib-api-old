...
describe("SetStoreBookCover endpoint", () => {
	...
	it("should create store book cover", async () => {
		...
		// Read the cover file and create the cover
	...
	it("should update store book cover", async () => {
		/*
		// Get the store book table object
		let storeBookTableObjectResponse;

		try{
			storeBookTableObjectResponse = await axios.default({
				method: 'get',
				url: `${constants.apiBaseUrl}/apps/object/${constants.authorUserAuthor.books[1].uuid}`,
				headers: {
					Authorization: constants.authorUserJWT
				}
			});
		...
		assert(storeBookTableObjectResponse.data.properties.cover != null)
		*/
